...
def _generate_reply(question, state, stopping_strings=None, is_chat=False, escape_html=False):
    ...
    if not is_chat:
        ...
        question = translate_text(apply_extensions('input', question, state),"eng")
        ...
        if is_stream:
            ...
            # Limit updates to 24 per second to not stress low latency networks
                ...
                if cur_time - last_update > 0.041666666666666664:
                    ...
                    yield translate_text(reply,"tr")

        if stop_found:
            break

    if not is_chat:
        reply = translate_text(apply_extensions('output', reply, state),"tr")
