...
def create_ui():
    ...
    with gr.Tab('Chat', elem_id='chat-tab'):
        ...
        with gr.Row():
            shared.gradio['Impersonate'] = gr.Button('Impersonate',visible=False)
            shared.gradio['Regenerate'] = gr.Button('Regenerate',visible=False)
            shared.gradio['Remove last'] = gr.Button('Remove last', elem_classes=['button_nowrap'],visible=False)

        with gr.Row():
            shared.gradio['Copy last reply'] = gr.Button('Copy last reply',visible=False)
            shared.gradio['Replace last reply'] = gr.Button('Replace last reply',visible=False)
        ...
        with gr.Row():
            shared.gradio['start_with'] = gr.Textbox(label='Start reply with', placeholder='Sure thing!', value=shared.settings['start_with'],visible=False)

        with gr.Row(visible=False):
            shared.gradio['mode'] = gr.Radio(choices=['chat', 'chat-instruct', 'instruct'], value=shared.settings['mode'] if shared.settings['mode'] in ['chat', 'instruct', 'chat-instruct'] else 'chat', label='Mode', info='Defines how the chat prompt is generated. In instruct and chat-instruct modes, the instruction templ ate selected under Parameters > Instruction template must match the current model.', elem_id='chat-mode',visible=True)
            shared.gradio['chat_style'] = gr.Dropdown(choices=utils.get_available_chat_styles(), label='Chat style', value=shared.settings['chat_style'])
