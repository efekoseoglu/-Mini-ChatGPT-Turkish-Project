...
import sys
sys.path.append('/home/grup02/workspace/ChaTR/modules/')

puncts_except_apostrophe = '!"#$%&\()*+,-./:;<=>?@[\\]^_`{|}~'
TOKENIZE_PATTERN = fr"[{puncts_except_apostrophe}]|\w+|['\w]+"
model_en = Seq2Seq.load("TranslateModel/model/mx-1.0.6/model.bin")
model_en.device = "cpu"
model_tr = Seq2Seq.load("TranslateModel/model/mx-1.0.5/model.bin")
model_tr.device = "cpu"
tokenizer = RegexpTokenizer(pattern=TOKENIZE_PATTERN)
def Translate_en(sentence_en):
    text_input = sentence_en
    tokenized_sent = tokenizer.tokenize(text_input)
    hypothesis = beam_search(model_en, [tokenized_sent], beam_size=1, max_decoding_time_step=70)[0]
    for idx, hyp in enumerate(hypothesis):
        return ' '.join(hyp.value)
def Translate_tr(sentence_tr):
    text_input = sentence_tr
    tokenized_sent = tokenizer.tokenize(text_input)
    hypothesis = beam_search(model_tr, [tokenized_sent], beam_size=1, max_decoding_time_step=70)[0]
    for idx, hyp in enumerate(hypothesis):
        return ' '.join(hyp.value)
...
def _generate_reply(question, state, stopping_strings=None, is_chat=False, escape_html=False):
    ...
    if not is_chat:
        ...
        question = Translate_tr(apply_extensions('input', question, state))
        ...
        if is_stream:
            ...
            # Limit updates to 24 per second to not stress low latency networks
                ...
                if cur_time - last_update > 0.041666666666666664:
                    ...
                    yield Translate_en(reply)

        if stop_found:
            break

    if not is_chat:
        reply = Translate_en(apply_extensions('output', reply, state))
