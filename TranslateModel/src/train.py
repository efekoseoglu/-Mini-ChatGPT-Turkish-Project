...
def train(model, optimizer, train_data, valid_data, args):
    ...
    n_epochsx = []       # epoch sayıları
    train_loss = []     # train loss değerleri
    valid_ppl = []      # valid perplexity değerleri
    ...
    while epoch_counter < n_epochs:
        ...
        epoch_counter += 1
    n_epochsx.append(epoch_counter+1)
    train_loss.append(train_loss)
    valid_ppl.append(valid_ppl)
    if (epoch_counter+1)%10==0:
        plt.figure(figsize=(10, 6))
        plt.plot(n_epochs, train_loss, label='Loss')
        plt.plot(n_epochs, valid_ppl, label='Valid_Ppl')
        plt.xlabel('Epoch')
        plt.ylabel('Value')
        plt.title(f'Loss and Valid_Ppl Over Epochs{(epoch_counter+1)}')
        plt.legend()
        plt.grid(True)
                # Grafikleri kaydet
        graph_filename = os.path.join(model_checkpoint_dir, f'loss_valid_ppl_graph_epoch_{(epoch_counter+1)}.png')
        plt.savefig(graph_filename)
        plt.close()  # Her 10 epoch sonrasında grafiği kaydet ve sıfırla
        
        # Modeli checkpoint olarak kaydet
        model_checkpoint = os.path.join(model_checkpoint_dir, f'checkpoint_epoch_{(epoch_counter+1)}.pt')
...
def main():
    ...
    arg_parser.add_argument("--lr_decay", type=float, default=0.250, 
